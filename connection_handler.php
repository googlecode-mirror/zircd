...
case 'user':
	...
	{ // You can only register once.
		...
		send($me, ':' . $config['name'] . ' 002 ' . $me['nick'] . ' :Your host is ' . $config['name'] . ', running version zircd v0.0.1 BETA');
		send($me, ':' . $config['name'] . ' 003 ' . $me['nick'] . ' :This server was created Wed Dec 22 12:30:25 2010');
		send($me, ':' . $config['name'] . ' 004 ' . $me['nick'] . ' ' . $config['name'] . ' zircd iowghraAsORTVSxNCWqBzvdHtGp lvhopsmntikrRcaqOALQbSeIKVfMCuzNTGj');
...
case 'version':
	send($me, ':' . $config['name'] . ' 005 ' . $me['nick'] . ' zirc-0.0.1 BETA ' . $config['name'] . ' FinWXOoZE3 [FreeBSD v8]');
